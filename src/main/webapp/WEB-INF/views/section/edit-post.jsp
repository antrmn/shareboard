            ...
            <div id = left-container class="selected-container">
                ...
                <div id = "post-data" class = "greyContainer">
                    <div class = "grid-x-nw" style = "flex-grow: 1">
                        <button id = "text-button" class = "post-type-button post-type-button-left ${requestScope.post.type == 'TEXT' ? "post-type-button-selected" : ""}" onclick = "togglePostType(this)">
                            <i class="fas fa-comment-alt" style = "display: inline"></i>
                            <p style = "display: inline">Post</p>
                        </button>
                        <button id = "image-button" class = "post-type-button post-type-button-right ${requestScope.post.type == 'IMG' ? "post-type-button-selected" : ""}" onclick = "togglePostType(this)">
                            <i class="fas fa-image"  style = "display: inline"></i>
                            <p style = "display: inline">Image</p>
                        </button>
                    </div>
                    <div id="action-container" style = "margin:8px">
                        <form id = "create-post-form" class = "grid-y-nw align-center justify-center" action="${pageContext.request.contextPath}/editpost" method="post" enctype="multipart/form-data">

                            <input type="text" id="title-field" class = "input-field" name="title" placeholder="Title" value="${requestScope.post.title}">
                            <textarea id="text-field" class = "input-field" name = "content" rows="5" placeholder="Text" >${requestScope.post.type == 'TEXT' ? requestScope.post.content : ""}</textarea>
